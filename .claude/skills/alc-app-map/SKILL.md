...
| **components** | `Tenko*.vue` (多数: Kiosk/VideoCall/RemoteAdminView/ScheduleManager 等) `*Dashboard.vue` `FaceAuth.vue` `AlcMeasurement.vue` `Device*.vue` `GwStatusCard.vue` | 点呼 UI / ダッシュボード / 測定 / デバイス管理 / GW 確認カード (DeviceSettings 内、GW 未検出時は折りたたみ) |
