---
name: resume-session
description: 前回セッションの引き継ぎを読み込み作業を自動再開する。引き継ぎ用 issue コメント (CCoW) または .claude/handoff.md から復元する。新セッション開始時や compact 後に実行。
---
...
1. **`$ARGUMENTS` に issue / comment の URL または番号が渡された場合** — 最優先。
   GitHub MCP の issue 取得 tool (例: `mcp__github__issue_read` / `get_issue`) で
   そのコメントを読む。これで handoff.md が無くても復元できる
