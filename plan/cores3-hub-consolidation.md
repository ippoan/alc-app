...
### セルラー通信 (LTE/CAT-M) の検討 (LAN代替/フォールバック、参考)

LAN Module 13.2 (Ethernet+PoE) をクラウド接続手段としているが、拠点にLAN配線が
無い場合のフォールバック/代替候補としてセルラー通信モジュールを調査した。

#### Module COMX LTE (SIM7600G, LTE Cat4)

参考: https://docs.m5stack.com/ja/module/comx_lte

- **対応バンド**: LTE-FDD B1/B2/B3/B4/B5/B7/B8/B12/B13/B18/B19/B20/B25/B26/B28/B66、
  LTE-TDD B34/B38/B39/B40/B41、WCDMA B1/B2/B4/B5/B6/B8/B19
- **日本キャリア照合**: ドコモ (B1/B3/B19/B28 対応、B21のみ非対応)、au
  (B1/B18/B26/B28/B41 対応、B11のみ非対応)、SoftBank (B1/B3/B8/B28/B41 全対応)。
  主要バンドはほぼ網羅
- **技適**: 公式ドキュメントの「認証」セクションに `CE/RCM/FCC/IC`、
  `PTCRB/GCF`、`AT&T/Verizon`、`RoHS/REACH` の記載はあるが、**日本の技適
  (技術基準適合証明) の記載が無い**。日本国内で電波を発射する用途で使う場合は
  技適の有無を個別に確認するか、技適未取得機器の届出制度の利用が必要

#### CAT-Mユニット (SIM7080G, LTE-M/NB-IoT)

参考: https://docs.m5stack.com/ja/unit/cat_m ,
https://www.switch-science.com/products/11052

- **対応バンド**: CAT-M B1/B2/B3/B4/B5/B8/B12/B13/B14/B18/B19/B20/B25/B26/B27/
  B28/B66/B85、NB-IoT B1/B2/B3/B4/B5/B8/B12/B13/B18/B19/B20/B25/B26/B28/B66/B71/B85
- **技適**: 製品ページに国内認証として **Softbank/Telec/JATE/Docomo/KDDI**
  が明記されており、Telec・JATEは日本の技適認証機関のため取得済みと判断できる。
  ただし**技適は付属の外付けSMAアンテナとセットで認証されている想定**であり、
  別アンテナに交換すると無効になる可能性がある点に注意
- **日本キャリア照合**: ドコモ系 (B19/B1/B28)、au (B18/B26/B28)、SoftBank
  (B8/B3/B28) いずれも対応。LTE-M/NB-IoT向けの省電力・低速通信用モジュール
  であり、通常のLTEデータ通信 (画像アップロード等) を高速に行う用途には不向き
- **楽天モバイル**: 自社回線バンド (B3/B28) 自体はモジュールがカバーするが、
  楽天モバイルがLTE-M/NB-IoT (Cat-M1) 網を商用提供しているか公式情報では
  未確認。Cat-M1専用SIMの提供有無を契約前に楽天モバイル法人窓口へ要確認

#### 結論・位置づけ

CoreS3統合ハブ化の主経路はLAN Module 13.2 (PoE) を維持する。セルラー通信は
あくまでLAN配線が無い拠点向けの代替/フォールバック候補であり、採用する場合は
Module COMX LTE (Cat4、技適要確認) よりもCAT-Mユニット (技適取得済み) の方が
即採用しやすい。ただしCAT-MはLTE-M/NB-IoT用の低速モジュールのため、想定用途
(点呼データ・アラート程度の軽量データ送信) に十分かは要検討。
