 ...
 * - **打刻から `FRESH_WINDOW_MS` (10 秒) より古い** — 立ち去った人のボタンを次の人が押してしまう
 ...
 *
 * **10 秒はユーザーの決定** (Refs ippoan/rust-alc-api#644)。以前は 60 秒で、
 * 仕掛け自体は効いていたが**実機で「いつまでも残る」と感じられた** —
 * かざした人はその場で押すので、60 秒は**立ち去った後も出続ける**長さだった。
 *
 * **`StrayAlcoholModal` の `AUTO_CLOSE_MS` (60 秒) とは別物。**あちらは
 * 「測りましたよ」と知らせるだけで、**押させるものではない**ので長くてよい。
 * かつて「同値」と書いてあったが、**揃える理由は無い**ので切り離した。
 */
...
// 何度も引き直されるので、引き直しのたびに測り直すと寿命が伸び続ける
