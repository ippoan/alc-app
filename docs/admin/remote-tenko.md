...
!!! note "ネットワーク要件"
    遠隔点呼にはカメラ・マイクの権限と安定したネットワーク接続が必要です。通信が切れると画面に「再接続」ボタンが表示されるので、乗務員がタップして再接続してください（自動では再接続しません）。

## 関連資料

国土交通省が定める遠隔点呼の実施要件（運用・機器システム・施設環境）は [遠隔点呼 要件チェックリスト](remote-tenko-checklist.md) を参照してください。
